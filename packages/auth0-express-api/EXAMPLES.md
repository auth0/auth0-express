...
> [!IMPORTANT]
> One code covers everything, including a missing client credential, which the other two exchanges report as `missing_client_auth_error`. So `error.cause` is what tells the cases apart, not the code.
...
Alongside `accessToken` and `expiresAt`, the result carries `scope`, `idToken`, `refreshToken`, `tokenType` and `issuedTokenType` when the tenant returns them. Ask for an `idToken` with the `openid` scope, and a `refreshToken` with `offline_access`.
...
> [!WARNING]
> When you pass `organization`, the SDK checks the returned ID token's `org_id` or `org_name` against what you asked for and throws if they differ. That check reads the ID token, so with no `openid` scope there is no ID token, and the check quietly does nothing. Ask for `scope: 'openid'` if you rely on it.
...
Same shape as [Handling a failed exchange](#handling-a-failed-exchange). Unlike Token Vault, this method reports a missing credential the ordinary way, so the two classes this SDK re-exports both apply:
...
| `organization_validation_error` | A blank `organization`, or an ID token naming a different one. Not exported either, so check the code. |
